(** The shapes an expression block desugars into.
    ...
    operator. The staged derivation mints those rules, and
    {!Rule.type-origin} names the role each rule came from.

    So this module decides how many rules a block turns into, and what each
    one is called.

    Every role is declared, whether or not the block uses it. {!is_active}
    says which ones a parse can produce. An inactive role still mints its
    kind constant, but it mints no rule. *)
