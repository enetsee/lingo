...
let scope_patterns (facts : Core.Facts.t) : string list =
  Array.to_list Core.Facts.(facts.rules)
  |> List.filter ~f:is_emitted
  |> List.filter_map ~f:(fun (rule : Core.Rule.def) ->
    if rule.opens_scope
    then Some (Printf.sprintf "(%s) @local.scope" (Node.of_rule rule.name))
    else None)
;;

let definition_patterns (facts : Core.Facts.t) : string list =
  Array.to_list Core.Facts.(facts.rules)
  |> List.filter ~f:is_emitted
  |> List.concat_map ~f:(fun (rule : Core.Rule.def) ->
    Array.to_list rule.binders
    |> List.filter_map ~f:(fun (index : int) ->
      let child = rule.children.(index) in
      match single child with
      | None -> None
      | Some kind ->
        (match pattern_for facts kind with
         | None -> None
         | Some inner ->
           Some
             (Printf.sprintf
...
let locals (scopes : Scopes.t) ~(word : Core.Token.def option) : string =
  let facts = Scopes.facts scopes in
  let references =
    match word with
    | None -> []
    | Some token -> [ Printf.sprintf "(%s) @local.reference" (Node.of_token token.name) ]
  in
  "; Generated by lingo from the grammar. Do not edit by hand.\n\
   ;\n\
  ...
  ^ section "a production that opens a scope" (scope_patterns facts)
  ^ section "a child position that introduces a name" (definition_patterns facts)
  ^ section
